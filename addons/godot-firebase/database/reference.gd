...
func _get_remaining_path(is_push : bool = true) -> String:
	var remaining_path = ""
	var auth_token = Firebase.Auth.auth.get("idtoken", "") if Firebase.Auth.auth.has("idtoken") else ""
	if _filter_query_empty() or is_push:
		remaining_path = _json_list_tag + _query_tag + _auth_tag + auth_token
	else:
		remaining_path = _json_list_tag + _query_tag + _get_filter() + _filter_tag + _auth_tag + auth_token
